...
unsigned long HalGPIO::getPowerButtonHeldTime() const { return inputMgr.getHeldTime(); } //removed "PowerButton"
...
void HalGPIO::verifyPowerButtonWakeup(uint16_t requiredDurationMs, bool shortPressAllowed) {
  ...
  if (inputMgr.isPressed(BTN_POWER)) {
    do {
      delay(10);
      inputMgr.update();
    } while (inputMgr.isPressed(BTN_POWER) && inputMgr.getHeldTime() < calibratedDuration); //removed "PowerButton"
    if (inputMgr.getHeldTime() < calibratedDuration) {  //removed "PowerButton"
      startDeepSleep();
    }
  } else {
    startDeepSleep();
