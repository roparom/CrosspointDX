...
void NoirTheme::drawBatteryRight(const GfxRenderer& renderer, Rect rect, bool showPercentage) const {
  // Reuse the generic implementation from BaseTheme to ensure consistent behavior
  BaseTheme::drawBatteryRight(renderer, rect, showPercentage);
}

// The base theme provides a generic fillBatteryIcon implementation that handles charging
// indicators and proper fill calculations. NoirTheme does not need a custom version, but
// the BaseTheme method is not automatically selected because the signature differs
// (BaseTheme expects a const GfxRenderer&, Rect, uint16_t). Provide a thin wrapper that
// forwards to the base implementation.
void NoirTheme::fillBatteryIcon(const GfxRenderer& renderer, Rect rect, uint16_t percentage) const {
  // Reuse the standard logic from BaseTheme and add a debug log to verify execution
  LOG_DBG("UI", "NoirTheme::fillBatteryIcon called with %u%%", percentage);
...
void NoirTheme::drawSideButtonHints(const GfxRenderer& renderer, const char* topBtn, const char* bottomBtn) const {
  ...
  for (int i = 0; i < 2; i++) {
    if (labels[i] != nullptr && labels[i][0] != '\0') {
      ...
      // BaseTheme::drawArrowIfNeeded requires a non-const renderer; cast away constness safely as the method does not modify logical state.
      if (!BaseTheme::drawArrowIfNeeded(const_cast<GfxRenderer&>(renderer), labels[i], x + buttonWidth / 2, y + buttonHeight / 2, 5, false)) {
        // White rotated text
        const int textWidth = renderer.getTextWidth(SMALL_FONT_ID, labels[i]);
...
void NoirTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
                                    const int selectorIndex, bool& coverRendered, bool& coverBufferStored,
                                    bool& bufferRestored, std::function<bool()> storeCoverBuffer) const {
  BaseTheme::drawRecentBookCover(renderer, rect, recentBooks, selectorIndex, coverRendered, coverBufferStored, bufferRestored, storeCoverBuffer);
}

void NoirTheme::drawEmptyRecents(const GfxRenderer& renderer, const Rect rect) const {
  // Simple placeholder for empty recents - render a message
  // This is rarely called since we typically have the recent books area even if empty
}
