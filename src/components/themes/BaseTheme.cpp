...
void BaseTheme::drawSpinner(const GfxRenderer& renderer, int centerX, int centerY, const char* text, int frame) const {
  // Draw a simple text-based spinner with animated dots
  // frame cycles 0-2, creating animated dots effect (. .. ...)
  
  // Draw text centered
  int textWidth = renderer.getTextWidth(UI_12_FONT_ID, text);
  int textY = centerY - renderer.getLineHeight(UI_12_FONT_ID) / 2;
  renderer.drawText(UI_12_FONT_ID, centerX - textWidth / 2, textY, text, true);
  
  // Draw animated dots below text
  const char* dots[] = {"•", "••", "•••"};
  int dotsWidth = renderer.getTextWidth(UI_12_FONT_ID, dots[2]);  // Use widest option
  int dotsY = textY + renderer.getLineHeight(UI_12_FONT_ID) + 8;
  
  const char* currentDots = dots[frame % 3];
  renderer.drawText(UI_12_FONT_ID, centerX - dotsWidth / 2, dotsY, currentDots, true);
}

bool BaseTheme::drawArrowIfNeeded(GfxRenderer& renderer, const char* label, int x, int y, int size, bool pointRight) {
  // Simple arrow implementation - draw a small arrow indicator next to the label
  // For now, this is a placeholder that always returns true (indicating arrow was drawn)
  // This can be expanded later with actual arrow rendering if needed
  return true;
}
