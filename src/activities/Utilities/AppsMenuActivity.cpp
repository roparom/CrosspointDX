...
void AppsMenuActivity::loop() {
  // Declare the activity pointer for the entire function so it is accessible for both confirm and back handling
  ...
  if (SETTINGS.uiTheme == CrossPointSettings::UI_THEME::RADAR) {
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
       // Reset any previous activity before creating a new one
       app.reset();
    ...
    // Back button: return to the main Crosspoint menu instead of being ignored
    if (mappedInput.wasReleased(MappedInputManager::Button::Back)) {
        // If we are in a submenu (app != nullptr), destroy it to go back to the main menu
        if (app) {
            app.reset();
            // Force a redraw of the main menu on next loop iteration
            requestUpdate();
        }
        // If already at main menu, fall through to power button handling (no action needed)
    }
    return;
  }
  ...
  // === CONFIRM: open category ===
   ...
   if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
       // Ensure any previous activity is cleared before creating a new one
       app.reset();
       switch (selectorIndex) {
        case 0: {
  ...
  if (selected) {
    ...
    switch (index) {
      ...
      case 5:  // GAMES
        ...
        break;
      case 6:  // MAIN MENU
        snprintf(statusStr, sizeof(statusStr), "Back to home");
        break;
      default:
        break;
    }
